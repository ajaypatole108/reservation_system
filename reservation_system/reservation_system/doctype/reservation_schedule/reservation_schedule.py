...
# This fuction close the reservation if valid_till date is over
def make_status_to_close_after_rs_validity_is_over():
	data = frappe.db.sql(f""" 
							SELECT name, reserve_till FROM `tabReservation Schedule`
							WHERE 
							status = 'Open'
						""",as_dict=1)
	for i in data:
	    if i.reserve_till != None:
	        if getdate(i.reserve_till) < getdate():
	            frappe.db.set_value('Reservation Schedule',i.name,'status','Close')
	            frappe.db.commit()
